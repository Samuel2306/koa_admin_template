...
class Model {
  ...
  static init(attributes, options = {}) {
    ...
    // 保存 sequelize 实例到 model 实例上面
    this.sequelize = options.sequelize;

    const globalOptions = this.sequelize.options;

    // 合并配置对象，globalOptions.define是sequelize为每个model定义的基础配置
    options = Utils.merge(_.cloneDeep(globalOptions.define), options);

    // 为model设置默认的名称，默认名称为'model'
    ...
    options = Utils.merge({
      name: {
        plural: Utils.pluralize(options.modelName),  // 复数名称
        singular: Utils.singularize(options.modelName)  // 单数名称
      },
      indexes: [], // 索引
      omitNull: globalOptions.omitNull, // 为真时，如果所有列中null，就不能保存
      schema: globalOptions.schema
    }, options);
    // 运行 beforeDefine 钩子函数
    ...
    this.options = {
      timestamps: true,  // 是否自动加上时间戳属性
      validate: {}, // 验证对象
      freezeTableName: false,
      underscored: false,  // 是否使用下划线的形式代替驼峰
      paranoid: false,  // 是否是偏执表，偏执表用的都是逻辑删除
      rejectOnEmpty: false,
    ...
    // this.underscored为true表示所有命名使用 _ 形式
  ...
  static refreshAttributes() {
    ...
    ['get', 'set'].forEach(type => {
      ...
      /**
       * 从每个属性中 或者 options对象的getterMethods和setterMethods属性中获取获取器和设置器函数，并将他们保存到_custom
       */
